...
resource "aws_iam_role" "github_actions_admin_role" {
    ...
    Statement = [
      {
        ...
        Condition = {
          StringLike = {
            "token.actions.githubusercontent.com:sub" = "repo:Albert166/CA-Devops:*"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy_attachment" "attach_policy" {
  role       = aws_iam_role.github_actions_admin_role.name
